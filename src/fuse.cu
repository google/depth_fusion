  ...
  for (int k = 0; k < regular_grid.depth(); ++k) {
    ...
    if (voxel_center_camera.z > 0 ||
        uv_int.x < 0 || uv_int.y < 0 ||
	      uv_int.x + 1 >= depth_data.size().x ||
        uv_int.y + 1 >= depth_data.size().y) {
      continue;
    }
    ...
    const int kDepthVarRadius = depth_var_radius;
    ...
    if (dz >= -max_tsdf_value) {
      ...
      // around surface.
      float weight = weight_multiplier * min(depth_var_param / std_d, 1.0f);
