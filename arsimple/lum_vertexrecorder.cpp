...
void VertexRecorder::record(vec3f pos,
    vec3f normal)
{
    record(pos, normal, vec3f(1, 1, 1));
}
void VertexRecorder::record_poscolor(vec3f pos,
    vec3f color) {
    record(pos, vec3f(0, 0, 0), color);
}
void VertexRecorder::record(vec3f pos,
    vec3f normal,
    vec3f color) {
...
void drawSphere(float r, int slices, int stacks) {
    ...
    for (int vi = 0; vi < stacks; ++vi) { // vertical loop
        ...
        for (int hi = 0; hi < slices; ++hi) { // horizontal loop
            ...
            vec3f p1(r * cosf(phi) * sinf(theta), r * sinf(phi) * sinf(theta), z);
            vec3f p2(r * cosf(phi_next) * sinf(theta), r * sinf(phi_next) * sinf(theta), z);
            vec3f p3(r * cosf(phi_next) * sinf(theta_next), r * sinf(phi_next) * sinf(theta_next), z_next);
            vec3f p4(r * cosf(phi) * sinf(theta_next), r * sinf(phi) * sinf(theta_next), z_next);

            vec3f n1 = p1.normalized();
            vec3f n2 = p2.normalized();
            vec3f n3 = p3.normalized();
            vec3f n4 = p4.normalized();
...
void drawCube(float w) {
    ...
    VertexRecorder rec;
    vec3f nx1 = vec3f(-wh, -wh, -wh);
    vec3f nx2 = vec3f(-wh, +wh, -wh);
    vec3f nx3 = vec3f(-wh, +wh, +wh);
    vec3f nx4 = vec3f(-wh, -wh, +wh);
    //rec.record(, );
    rec.record(vec3f(-wh, wh, -wh), vec3f(-1, 0, 0));
    rec.record(vec3f(-wh, wh, wh), vec3f(-1, 0, 0));


    vec3f px1 = vec3f(+wh, -wh, -wh);
    vec3f px2 = vec3f(+wh, +wh, -wh);
    vec3f px3 = vec3f(+wh, +wh, +wh);
    vec3f px4 = vec3f(+wh, -wh, +wh);
...
void drawCylinder(int nsides, float r, float h) {
    ...
    for (int face = 0; face < nsides; ++face) {
        ...
        pos.push_back(vec3f(lx, 0.0f, lz));
        pos.push_back(vec3f(lx, h, lz));

        n.push_back(vec3f(cosf(face * step), 0.0f, sinf(face * step)));
        n.push_back(vec3f(cosf(face * step), h, sinf(face * step)));
...
void drawQuad(float w)
{
    ...
    float wh = w / 2;
    const vec3f N(0, 1, 0);
    const vec3f P1(-wh, 0, -wh);
    const vec3f P2(+wh, 0, -wh);
    const vec3f P3(+wh, 0, +wh);
    const vec3f P4(-wh, 0, +wh);
