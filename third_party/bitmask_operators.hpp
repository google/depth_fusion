...
// TODO: why does the enable_if version not work?
#if 1
template<typename E>
bool isZero(E lhs){
    typedef typename std::underlying_type<E>::type underlying;
...
template<typename E>
bool isSet(
  typename std::enable_if<enable_bitmask_operators<E>::enable,E>::type value,
  typename std::enable_if<enable_bitmask_operators<E>::enable,E>::type query) {
    return notZero(E & query);
}

template<typename E>
bool isZero(
  typename std::enable_if<enable_bitmask_operators<E>::enable,E>::type lhs){
    typedef typename std::underlying_type<E>::type underlying;
    return static_cast<underlying>(lhs) == static_cast<underlying>(0);
}
